...
void ShadeTreeNode::_evaluateShading(const Shading& shading0, const Shading& shading1, const std::string& expression, Shading* result, std::string* node_glsl_code)
{
   const std::string& output_slot = "Shader";
    ...
    {
        *node_glsl_code += "vec3 " + _toGLSLVarName(name, output_slot) + " = " + string_format_str(expression, shading0.additive, shading1.additive) + ";\n";
    ...
    if (shading0.has_transparency_factor || shading1.has_transparency_factor)
    {
        *node_glsl_code += "vec3 " + _toGLSLVarName(name, output_slot, true) + " = "
            + string_format_str(expression, shading0.transparency_factor, shading1.transparency_factor) + ";\n";
...
const NodeEvaluatedOutputs& TexImageNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
    ...
    if (!compute_pixel_differentials)
    {
       ...
       node_glsl_code += string_format_str("sampleTexture(%s, vec3(attr_uv, 1.0), %s, %s, %s);\n",
    ...
    else
    {
       ...
       node_glsl_code += string_format_str("sampleTextureDifferentials(%s, vec3(attr_uv, 1.0), %s, %s, %s, %s, %s, %s, %s);\n",
...
const NodeEvaluatedOutputs& NormalMapNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code = string_format_str("vec3 %s = evalNormalMap(%s, %s);\n", glsl_normal, glsl_color, glsl_strength);
...
const NodeEvaluatedOutputs& BumpNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code = string_format_str("vec3 %s = evalBump(%s, %s, %s, %s, vec2(%s, %s), %s);\n",
                                       ...
                                       const std::string& value2)
{
   ...
   if (operation == "ADD")
      node_glsl_code += string_format_str(" %s = %s + %s;\n", output, value1, value2);
      ...
      node_glsl_code += string_format_str(" %s = sin(%s);\n", output, value1);
   else if (operation == "COSINE")
      node_glsl_code += string_format_str(" %s = cos(%s);\n", output, value1);
   else if (operation == "TANGENT")
      node_glsl_code += string_format_str(" %s = tan(%s);\n", output, value1);
   else if (operation == "ARCSINE")
      node_glsl_code += string_format_str(" %s = asin(%s);\n", output, value1);
   else if (operation == "ARCCOSINE")
      node_glsl_code += string_format_str(" %s = acos(%s);\n", output, value1);
   else if (operation == "ARCTANGENT")
      node_glsl_code += string_format_str(" %s = atan(%s);\n", output, value1);
   else if (operation == "POWER")
      node_glsl_code += string_format_str(" %s = pow(%s, %s);\n", output, value1, value2);
   else if (operation == "LOGARITHM")
      node_glsl_code += string_format_str(" %s = log(%s, %s);\n", output, value1, value2);
   else if (operation == "MINIMUM")
      node_glsl_code += string_format_str(" %s = min(%s, %s);\n", output, value1, value2);
   else if (operation == "MAXIMUM")
      node_glsl_code += string_format_str(" %s = max(%s, %s);\n", output, value1, value2);
   else if (operation == "ROUND")
      node_glsl_code += string_format_str(" %s = floor(%s+0.5);\n", output, value1);
      ...
      node_glsl_code += string_format_str(" %s = abs(%s);\n", output, value1);

   if (clamp)
      node_glsl_code += string_format_str("%s = clamp(%s, 0.0, 1.0);\n", output, output);
...
const NodeEvaluatedOutputs& FresnelNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code = string_format_str("float %s = evalFresnel(%s, %s);\n", output, ior, value2);
   NodeEvaluatedOutputs& result = evaluation.addNodeCode(name, node_glsl_code);
...
const NodeEvaluatedOutputs& MixRGBNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code;
   if (operation == "MIX")
      node_glsl_code += string_format_str("vec3 %s = node_mix_blend(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "ADD")
      node_glsl_code += string_format_str("vec3 %s = node_mix_add(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "MULTIPLY")
      node_glsl_code += string_format_str("vec3 %s = node_mix_mul(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "SCREEN")
      node_glsl_code += string_format_str("vec3 %s = node_mix_screen(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "OVERLAY")
      node_glsl_code += string_format_str("vec3 %s = node_mix_overlay(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "SUBTRACT")
      node_glsl_code += string_format_str("vec3 %s = node_mix_sub(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "DIVIDE")
      node_glsl_code += string_format_str("vec3 %s = node_mix_div(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "DIFFERENCE")
      node_glsl_code += string_format_str("vec3 %s = node_mix_diff(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "DARKEN")
      node_glsl_code += string_format_str("vec3 %s = node_mix_dark(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "LIGHTEN")
      node_glsl_code += string_format_str("vec3 %s = node_mix_light(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "DODGE")
      node_glsl_code += string_format_str("vec3 %s = node_mix_dodge(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "BURN")
      node_glsl_code += string_format_str("vec3 %s = node_mix_burn(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "HUE")
      node_glsl_code += string_format_str("vec3 %s = node_mix_hue(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "SATURATION")
      node_glsl_code += string_format_str("vec3 %s = node_mix_sat(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "VALUE")
      node_glsl_code += string_format_str("vec3 %s = node_mix_val(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "COLOR")
      node_glsl_code += string_format_str("vec3 %s = node_mix_color(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "SOFT_LIGHT")
      node_glsl_code += string_format_str("vec3 %s = node_mix_soft(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);
   else if (operation == "LINEAR_LIGHT")
      node_glsl_code += string_format_str("vec3 %s = node_mix_linear(clamp(%s, 0.0, 1.0), %s, %s);\n", output, fac, color1, color2);

   if (clamp)
      node_glsl_code += string_format_str("%s = clamp(%s, vec3(0.0), vec3(1.0));\n", output);
...
const NodeEvaluatedOutputs& ColorRampNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code = string_format_str("vec3 %s = texture(%s, %s).rgb;\n", glsl_color, glsl_ramp, glsl_fac);
...
const NodeEvaluatedOutputs& RgbToBWNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   std::string node_glsl_code = string_format_str("float %s = linearRgbToGray(%s);\n", glsl_out_val, glsl_color);
...
const NodeEvaluatedOutputs& LayerWeightNode::evaluate(const ShadeTreeParams& params, ShadeTreeEvaluation& evaluation)
{
   ...
   node_glsl_code += string_format_str("evalLayerWeight(%s, %s, %s, %s);\n", glsl_blend, glsl_normal, glsl_out_fresnel, glsl_out_facing);
