...
void Skeleton::_updateBone(int index, const mat4x4 skeleton_to_parent_matrix)
{
   ...
   _skeleton_to_bone_pose_matrices[index] = composeAS(skeleteon_to_current_bone, toMat4x3(translate(bone_pose.location)*mat4_cast(bone_pose.quaternion)*scale(bone_pose.scale)));
