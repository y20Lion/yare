...
void main()
{
   ...
   for (int z = 0; z < volume_size.z; ++z)
   {
      ...
      vec3 froxel_in_view = convertFroxelToCameraCoords(vec3(gl_GlobalInvocationID.xy, z), volume_size);
      vec3 froxel_center_in_world = mat4x3(matrix_world_view) * vec4(froxel_in_view, 1.0);

      vec3 froxel_coords01 = vec3(gl_GlobalInvocationID.xy, z)/volume_size;
