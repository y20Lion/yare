...
Uptr<GLTextureCubemap> CubemapConverter::createDiffuseCubemap(const GLTextureCubemap& input_cubemap) const
{
   ...
   GLFramebufferDesc desc;
   desc.attachments.push_back(GLGLFramebufferAttachmentDesc{ diffuse_cubemap_without_deleter, GL_COLOR_ATTACHMENT0, 0 });
   GLFramebuffer render_to_cubemap(desc);
   ...
   // That makes a total of 603 979 776 texture reads, which is done in 190ms (I guess we can thank Mr cache)
   for (int i = 0; i < 6; ++i)
   {
      GLDevice::bindFramebuffer(&render_to_cubemap, i);
